...
files_22  = glob(f'{BASE_PATH}/2022/*/*.ogg')
files_23 = glob(f'{BASE_PATH}/2023/*/*.ogg')

new_labels = []
for file in files_22:
    target = file.split('/')[2]
    new_labels.append({'filepath': file, 'primary_label': target})

df22 = pd.DataFrame(new_labels)

new_labels = []
for file in files_23:
    target = file.split('/')[2]
    new_labels.append({'filepath': file, 'primary_label': target})
df23 = pd.DataFrame(new_labels)
...
df22['target'] = df22.primary_label.map(CFG.name2label)
df22.dropna(subset=['target'], inplace=True)
df22['target'] = df22['target'].astype(int)

df23['target'] = df23.primary_label.map(CFG.name2label)
df23.dropna(subset=['target'], inplace=True)
df23['target'] = df23['target'].astype(int)


df = pd.concat([df, df22, df23], ignore_index=True)
...
def build_augmenter(noise_factor=0.1):
    augmenters = [
    ...
    def augment(img, label):
        ...
        #     data["images"] = noising(data["images"], 0.01)
...
train_df, valid_df = train_test_split(df, test_size=0.02)
...
ckpt_cb = keras.callbacks.ModelCheckpoint("best_model_val_loss.weights.h5",
                                         monitor='val_loss',
                                         save_best_only=True,
